...
> 2026-07-02 由 Claude 整理。目的：没有 AI 在场时，你自己也能推进、能学到东西。
...
2. 渲染 16×16 1bpp 字形：抄 `test_inject_e0.py` 里的 `render_glyph()`（PIL + wqy-zenhei，
   32 字节/字，2 字节一行 MSB 在左），写进 `FONT.BIN` 的 `slot*32` 处；
...
**验证**：渲染预览（抄 `test_inject_e0.py` 的 `render_line`）+ 实机抽查，
确认没有 `0/123?` 那种折行。
