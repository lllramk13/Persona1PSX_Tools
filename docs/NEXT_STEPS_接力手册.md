...
### T0.5 ★★ 字库扩容到 4096（当前主线；决定后续一切，详见 §10）

**为什么先做**：全游戏中文需 ~3111 unique 字（Heaps 外推），2048 装不下（README §1「字库容量」）。
先把码表天花板从 2048 抬到 4096，管道和翻译才不用中途返工。**这是独立逆向子项目**，
完整拆解见 **§10**。核心结论速记：
...
**注意**：T0.5 与 T1-T5 解耦——不想等它也可以先用 2048 跑通 E0（腾退未用槽，~1800 字够）。
路线选择见 §0 / 用户已定"先啃 4096"。
...
若 T0.5 已完成，槽位空间是 4096；否则只有 2048（先跑 E0 够用）。
...
## 10. 字库扩容 4096 子项目（T0.5 详解，当前主线）

> 结论性事实见 README §1「字库容量与 4096 扩容」。这里是**动手拆解**。
