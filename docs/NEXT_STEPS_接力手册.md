...
| 翻译进度 | `Text/translations.json` 已有 E0 等译文；以脚本实时统计为准 |
| 术语表 | 7877 候选，0 已定名 ⚠ |
| E0 多入口/breakpoint 回插 | 🔄 已能迁移；复杂对话排版仍未完成 |
| 镜像扩容层（E 文件变大时） | ✅ E0 盘尾追加 + FSECT/FSIZE + ISO/PVD 已完成 |
| 静态 4096 字库 | ❌ 7 份 RAM 快照否决，没有 64/128KB 常驻空洞 |
| 动态分页 FF5B | ⏸ **实验暂停**：静态通过、实机会破坏状态并卡死 |
| 当前稳定主线 | 固定 2048 字库 + E0 回插 + 独立 format 工具 |
| git | ⚠ **大量成果未提交，第一件事就是 commit** |

> 动态分页实验的完整失败记录见 README“当前快照”和“字库容量”章节。没有 DuckStation
> breakpoint/寄存器证据前，不要继续制作 FF5B 测试盘，也不要把 `dynamic_font` 产物混入稳定盘。
...
### T0.5 字库扩容到 4096（已否决；历史方案，详见 §10）

全游戏中文需 ~3111 unique 字，容量问题依然存在；但 7 份运行时快照已经证明没有可供静态
4096 字库常驻的 64/128KB 空洞，因此本项不是当前任务。以下内容仅保留为历史逆向记录：
- 解码 `& 7`→`& 0xF`（1 字节）启用逃逸头 0x80-0x8F = 4096；绘制数学已支持 4095；
- 真正难点 = 找一块**所有模式都空的连续 128KB**（静态分析做不到，需 DuckStation 运行时观测）
  + 改 ~57 处硬编码基址 `0x801E0000` + FONT.BIN 翻倍。

当前路线见 §0：固定 2048 字库先完成 E0；动态分页 FF5B 也已暂停。

### T1 中文字库：字形写入 + 码表演进（M2 第一块，难度 ★★☆）

**目标**：一个 `font_append.py`——输入若干中文字，输出 patch 后的 FONT.BIN 和
追加了新字的 codetable JSON。**只追加，不动任何原槽位**（码表=存档 ABI）。
当前槽位空间固定为 2048，先做 E0 阶段的字集和缺字报告。
...
## 10. 字库扩容 4096 子项目（历史记录，已否决）

> 结论性事实见 README 字库容量章节。本节候选方案均不是当前主线；静态方案已被 RAM 转储否决，
> 动态 FF5B 实验也因实机卡死暂停。

### 10.1 已经钉死的事实（反汇编实锤，不用重查）

| 项 | 地址 | 内容 |
|---|---|---|
| 逃逸解码 | ADV.BIN `0x80067794` | `andi $s0,7` → slot 页号只 3 bit。改 `07`→`0F` = 4 bit = 4096 |
| 字形绘制 | ADV.BIN `0x80067924` | `addr = 0x801E0000 + slot*32`，slot 16 位掩码，无 2048 检查 |
| 字库基址 | 硬编码 `lui 0x801e` | ADV×5 DNG×28 S2D×22 BTLP×2 SLPS×1 ≈ 57 处 |
| 加载器 | SLPS `0x80011E3C` | 描述符 `0x800100B4`=`\FONT.BIN;1`，dest=`$a1`，由 SLPS 文件偏移 `0x1BEC` 的 lui 设定 |
| 冲突 | `0x801F0000+` | RAM 顶，游戏状态密集区；字库 64KB 正好顶到这里，翻倍会压穿 |

### 10.2 三个候选方案（先定方向再动手）

**A. 基址下移（最干净，若能找到空间）**：把字库整体搬到 `0x801D0000`（或更低），
占 `0x801D0000-0x801EFFFF`（128KB），仍在 `0x801F0000` 游戏状态之下。改动 = 加载器 lui +
57 处绘制基址 lui，全部 `0x801e`→`0x801d`。**前提**：`0x801D0000-0x801DFFFF` 在所有模式空闲。

**B. 分离第二缓冲（省一半 RAM）**：页 0-7 留 `0x801E0000` 不动，页 8-15（新中文）放另一块
64KB，绘制函数注入 `if slot>=2048: base=B2` 的 detour。只需 64KB 空闲，但要改绘制逻辑（更精细）。

**C. 分模式 bank（不扩 RAM，救急）**：对话/战斗各加载不同的 2048 页集。物理仍 2048，
存档 ABI 风险大，最后手段。

### 10.3 关键障碍：找空闲 128KB（静态分析做不到）

**为什么静态不行**：大缓冲区（字库/堆/栈）走 `base+index` 计算地址，`lui` 扫描看不见。
本会话实测：对 0x801C-0x801F 扫完整地址，字库自己的区间 `0x801E0000-0x801EFFFF` 被误报成
多个"空洞"——所以扫出来的空洞**不可信**。

**可靠办法 = DuckStation 运行时观测**（需要你动手）：
1. DuckStation 有内存查看/调试（或用 no$psx / pcsx-redux 的 RAM watch）；
2. 跑遍所有模式（开场对话、迷宫、战斗、赌场、菜单、地图），记录 `0x801C0000-0x801EFFFF`
   哪些字节**全程从未被写**；
3. 找出其中连续 128KB（方案 A）或 64KB（方案 B）恒空的区间 = 新字库家。
4. 稳妥起见：候选地址先做"占位实验"——往那片写一个哨兵图案，跑全流程，回来看它有没有被
   游戏覆盖；没被覆盖才安全。

### 10.4 落地步骤（方案 A 为例）

1. **font_expand.py**：FONT.BIN（64KB）→ 128KB（后 64KB 先填 0，留给页 8-15 的中文）；
2. **patch_font_base.py**：把 6 个二进制里所有 `lui rX,0x801e`（字库基址那些，
   **别误伤非字库的 0x801e 引用**——先用完整地址确认低位是 0x0000/字形偏移）改成新基址；
   加载器 SLPS `0x1BEC` 一并改；
3. **decode 补丁**：ADV.BIN `0x80067794` = 文件偏移 `0x33F4`，`andi $v0,$s0,7`
   机器码 `07 00 02 32`；**只改 1 字节**：`0x33F4` 的 `07`→`0F`（变成 `andi $v0,$s0,0xF`）；
4. 三个补丁都属于 overlay/EXE，走各自的盘内 LBA 原位等长写回（FONT 变长走扩容，
   代码补丁等长）；
5. **验证盘**：在新页（如 slot 2048）写一个醒目字形，找一句已知文本临时指到它，
   实机确认①新字渲染出来②游戏全流程不崩（证明新址真空闲、基址全改对了）。

### 10.5 验证清单

- [ ] 运行时确认候选 128KB 全程无写入（占位哨兵实验）
- [ ] 57 处基址全部改对：改完后**原有 2048 字**显示必须完全正常（漏一处就花屏）
- [ ] decode 改对：`0x88 XX` 现在应解成 slot `0x8XX`（页 8），不再别名回页 0
- [ ] 新页写字 → 实机渲染正确
- [ ] 存档兼容：老存档（存 slot 码）载入不乱（页 0-7 未动即安全）

### 10.6 你会学到

overlay 内存布局、跨二进制一致性补丁、运行时内存分析（静态的边界）、
PS-X EXE 加载机制、为什么"找空闲 RAM"是逆向里最需要动态手段的一类问题。
...
新会话开头直接说：
> 读 P1_Tools/README.md 的 §3.2b、§1「字库容量」、docs/NEXT_STEPS_接力手册.md（§10）
> 和 docs/RE_Walkthrough_EBIN.md，我们从 T〈编号〉继续。
